...
save_filename='random_number/'#文件保存位置
font_size=15#字体大小
file_txt='number1_generator.txt'#保存ｔｘｔ文件名字

for i in range(num_len):
    font_size=15
    random_number1 = "".join([random.choice(characters) for j in range(0, random.choice(range(1,5)))])
    print(random_number1[0])
    ...
    path=filename+datetime.datetime.now().strftime('%Y-%m-%d')+'_number_'+str(i)+'_'+str(i)+'.png'
    number = len(random_number)
    gene_code(random_number, path, number, font_size)
    with open('number_gen.txt', 'a') as f:
        f.write(path+'\n'+random_number+'\n')
