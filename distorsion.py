...
class DistorsionGenerator(object):
    ...
    def apply_func_distorsion(cls, image, vertical, horizontal, max_offset, func):
        ...
        if horizontal:
            ...
            for i, o in enumerate(horizontal_offsets):
                if vertical:
                    new_img_arr_copy[i, max_offset + o:row_width + max_offset + o, :] = new_img_arr[i,
                                                                                        max_offset:row_width + max_offset,
                                                                                        :]
                else:
    ...
    def sin(cls, image, vertical=False, horizontal=False):
        ...
        a = cls.apply_func_distorsion(image, vertical, horizontal, max_offset,
                                      (lambda x: int(math.sin(math.radians(x)) * max_offset)))
        plt.imshow(a)
        plt.show()
        return cls.apply_func_distorsion(image, vertical, horizontal, max_offset,
                                         (lambda x: int(math.sin(math.radians(x)) * max_offset)))
    ...
    def random(cls, image, vertical=False, horizontal=False):
        ...
        return cls.apply_func_distorsion(image, vertical, horizontal, max_offset,
                                         (lambda x: random.randint(0, max_offset)))
